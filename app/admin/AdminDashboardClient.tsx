    ...
    >
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard title="Total Conversions" value={totalJobs} icon={FileText} />
          <StatCard
            title="Success Rate"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link href="/admin/users">
              <div className="p-6 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A2332] hover:border-cyan-400/50 hover:shadow-md transition-all cursor-pointer">
                ...
                <h3 className="font-display font-semibold text-slate-900 dark:text-white mb-2">Manage Users</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Create accounts, edit description documents, manage templates</p>
              </div>
            </Link>
            <Link href="/admin/jobs">
              <div className="p-6 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A2332] hover:border-cyan-400/50 hover:shadow-md transition-all cursor-pointer">
                <FileText className="w-8 h-8 text-cyan-600 dark:text-cyan-400 mb-3" />
                <h3 className="font-display font-semibold text-slate-900 dark:text-white mb-2">View All Jobs</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Monitor conversion queue, review errors, download outputs</p>
              </div>
            </Link>
            <Link href="/dashboard">
              <div className="p-6 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A2332] hover:border-cyan-400/50 hover:shadow-md transition-all cursor-pointer">
        ...
        <section>
          ...
          {recentJobs.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-[#1A2332] rounded-xl border border-slate-200 dark:border-white/10">
              ...
              <p className="text-slate-600 dark:text-slate-400">No conversions yet</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-[#1A2332] rounded-xl border border-slate-200 dark:border-white/10 overflow-hidden">
              <table className="w-full">
                <thead className="border-b border-slate-200 dark:border-white/10">
                  <tr>
                    <th className="text-left px-6 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider">Job ID</th>
                    <th className="text-left px-6 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider">Status</th>
                    <th className="text-left px-6 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider">Created</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                  {recentJobs.map((job) => (
                    <tr key={job.id} className="hover:bg-slate-50 dark:hover:bg-white/5">
                      <td className="px-6 py-4 text-sm font-mono text-slate-900 dark:text-white">{job.id.slice(0, 8)}...</td>
                      <td className="px-6 py-4 text-sm">
                        <Badge variant={job.status === 'complete' ? 'success' : job.status === 'processing' ? 'warning' : job.status === 'failed' ? 'error' : 'default'}>
                          {job.status}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{new Date(job.created_at).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
